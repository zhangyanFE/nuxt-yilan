...
export const getDeviceData = function() {
    let DeviceData = {}
    DeviceData.imei = getAddressKey('imei') ? getAddressKey('imei') : (getAddressKey('simei') ? getAddressKey('simei') : (getAddressKey('c_imeimd5') ? getAddressKey('c_imeimd5') : ''));
    DeviceData.imeimd5 = getAddressKey('imeimd5') ? getAddressKey('imeimd5') : (getAddressKey('simeimd5') ? getAddressKey('simeimd5') : (getAddressKey('c_imeimd5') ? getAddressKey('c_imeimd5') : ''));
    DeviceData.mac = (getAddressKey('mac') ? getAddressKey('mac') : (getAddressKey('smac') ? getAddressKey('smac') : (getAddressKey('c_mac') ? getAddressKey('c_mac') : ''))).toUpperCase();

    if (DeviceData.mac && DeviceData.mac.indexOf(':') == -1) {
        DeviceData.mac = str_to_mac(DeviceData.mac)
    }
    DeviceData.adid = getAddressKey('adid') ? getAddressKey('adid') : (getAddressKey('sadid') ? getAddressKey('sadid') : (getAddressKey('c_adid') ? getAddressKey('c_adid') : ''));
    DeviceData.idfa = getAddressKey('idfa') ? getAddressKey('idfa') : (getAddressKey('sidfa') ? getAddressKey('sidfa') : (getAddressKey('c_idfa') ? getAddressKey('c_idfa') : ''));
    return DeviceData
}
...
export const getMessageData = function() {
    let MessageData = {}
        // 设备品牌
    MessageData.brand = getAddressKey('brand') ? getAddressKey('brand') : (getAddressKey('sbrand') ? getAddressKey('sbrand') : '');
    // 设备型号
    MessageData.model = getAddressKey('model') ? getAddressKey('model') : (getAddressKey('smodel') ? getAddressKey('smodel') : '');
    // 网络环境 int 1-WIFI，2-5G以上，3-2G，4-3G，5-4G，0-未知
    MessageData.nt = getAddressKey('nt') ? getAddressKey('nt') : (getAddressKey('snt') ? getAddressKey('snt') : '');
    // 运营商 int 运营商 70120-移动，70121-电信，70123-联通，0—其他
    MessageData.telecom = getAddressKey('telecom') ? getAddressKey('telecom') : (getAddressKey('stelecom') ? getAddressKey('stelecom') : '');
    // 操作系统版本，如7.0
    MessageData.os_ver = getAddressKey('os_ver') ? getAddressKey('os_ver') : (getAddressKey('sos_ver') ? getAddressKey('sos_ver') : '');
    return MessageData
}
export const getDeviceTwoData = function() {
    let DeviceTwoData = {}
    DeviceTwoData.bs = getAddressKey('bs') ? getAddressKey('bs') : (getAddressKey('c_bs') ? getAddressKey('c_bs') : '');
    DeviceTwoData.nw = getAddressKey('nw') ? getAddressKey('nw') : (getAddressKey('c_nw') ? getAddressKey('c_nw') : '');
    DeviceTwoData.cityid = getAddressKey('cityid') ? getAddressKey('cityid') : (getAddressKey('c_cityid') ? getAddressKey('c_cityid') : '');
    DeviceTwoData.strategy = getAddressKey('strategy') ? getAddressKey('strategy') : (getAddressKey('c_strategy') ? getAddressKey('c_strategy') : '');
    DeviceTwoData.category = getAddressKey('category') ? getAddressKey('category') : (getAddressKey('c_category') ? getAddressKey('c_category') : '');
    DeviceTwoData.s = getAddressKey('s') ? getAddressKey('s') : (getAddressKey('c_s') ? getAddressKey('c_s') : '');
    DeviceTwoData.columns_id = getAddressKey('columns_id') ? getAddressKey('columns_id') : (getAddressKey('c_columns_id') ? getAddressKey('c_columns_id') : '');
    DeviceTwoData.v = getAddressKey('v') ? getAddressKey('v') : (getAddressKey('c_v') ? getAddressKey('c_v') : '');
    DeviceTwoData.stats_referer = getAddressKey('stats_referer') ? getAddressKey('stats_referer') : (getAddressKey('c_stats_referer') ? getAddressKey('c_stats_referer') : '');
...
export function conversionUnit(num) {
    return num > 10000 ? Math.floor((num) / 100) / 100 + '万' : num
}

// 埋点上报
export function sendSpm(params) {
    const { spmID, type, videoid = '', eventName = '', data } = params;
    window.YLSPM && window.YLSPM.RecordEvent(spmID, {
        type,
        videoid,
...
export function sendJsBridge(event, data) {
    if (curClient() == "iPhone") {
        if (window.webkit) {
            window.webkit.messageHandlers[event] && window.webkit.messageHandlers[event].postMessage(data);
        }
    }
